...
type ControlPaths<FormAccessorFormType extends AbstractControl, Depth extends number = 10> = [Depth] extends [never]
	? never
	: FormAccessorFormType extends FormGroup
	? {
			[Key in keyof FormAccessorFormType['controls']]-?: Key extends string | number
				? `${Key}` | Join<Key, ControlPaths<FormAccessorFormType['controls'][Key], Previous[Depth]>>
				: never;
